...
public class CommonTabLayout extends FrameLayout implements ValueAnimator.AnimatorUpdateListener {
    ...
    private int mHeight;
    private boolean followTvWidth=false;//指示器的宽度是否以内容而定,默认否
    private int mCenterPublishIcon;//发布按钮的图片
    private int mPublishMode;
    private boolean isHasCenterPublish;
    ...
    private ArrayList<CustomTabEntity> mTabEntitys = new ArrayList<>();//数据
    private int mTabCount;
    private int mCurrentTab;
    private int mLastTab;
    private float mTabPadding;
    private boolean mTabSpaceEqual;//是否均分
    private float mTabWidth;

    private static final int STYLE_NORMAL = 0;
    private static final int STYLE_TRIANGLE = 1;//三角形
    private static final int STYLE_BLOCK = 2;//块
    ...
    private static final int TEXT_BOLD_WHEN_SELECT = 1;//选中时加粗
    private static final int TEXT_BOLD_BOTH = 2;//都加粗
    private float mTextUnSelectsize;
    private float mTextSelectSize;
    private int mTextSelectColor;
    private int mTextUnselectColor;
    private int mTextBold;
    private boolean mTextAllCaps;//是否小写转大写，针对英文有效

    public void setmTextSelectSize(int mTextSelectSize) {
    ...
    public CommonTabLayout(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        //定义他的子控件是否要在他应有的边界内进行绘制。false允许进行扩展绘制
        setClipChildren(false);
        //定义ViewGroup是否允许在padding中绘制。默认情况下被设置为ture,也就是把padding中的值都进行裁切了。
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mTextUnSelectsize = ta.getDimension(R.styleable.CommonTabLayout_tl_textsize, sp2px(13f));
        ...
        mPublishMode = ta.getInt(R.styleable.CommonTabLayout_tl_publishMode, 0);
        mCenterPublishIcon=ta.getResourceId(R.styleable.CommonTabLayout_tl_centerPublishIcon,R.drawable.ic_center_publish);
        ta.recycle();
    }
    //设置数据源
    public void setTabData(ArrayList<CustomTabEntity> tabEntitys) {
        ...
        if (mPublishMode == 2) {
            if (tabEntitys.size() % 2 == 0){
                ...
                tabEntitys.add(index, tabEntity);
                isHasCenterPublish=true;
            }else {
                isHasCenterPublish=false;
            }
        }
    ...
    private void addTab(final int position,final View tabView) {
        ...
        tabView.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (secondClickTime-firstClickTime<1000&&position==mCurrentTab){
                    firstClickTime=0;
                    secondClickTime=0;
                    if (mListener != null) {
                        //发布图片不显示的情况下有双击事件
                        if (mPublishMode != 1||publish.getVisibility()!=View.VISIBLE){
                            mListener.onDoubleClick(position);
                        }
                    }
                }
                // 点击了发布按钮固定在中间的情况(如果奇数个tab并设置了mPublishMode = 2不加isHasCenterPublish判断会走次方法造成错误的点击事件)
                if (mPublishMode == 2 && (position == mTabCount/2)&&isHasCenterPublish) {
                    if (mListener != null) {
        ...
        //此段可使得覆盖图片比tab高
        if (mPublishMode == 1){
            RelativeLayout rl = tabView.findViewById(R.id.rl_tap);
...
//            tabView.setPadding((int) mTabPadding, (int) mTabPadding, (int) mTabPadding, (int) mTabPadding);
            ...
            ImageView iv_tab_icon = tabView.findViewById(R.id.iv_tab_icon);
            ImageView publishIv = tabView.findViewById(R.id.publish);

            //是否有覆盖图片
            if (mPublishMode == 1 && i == mCurrentTab && mTabEntitys.get(i).getCoverIcon() != 0) {
                publishIv.setVisibility(VISIBLE);
                publishIv.setImageResource(mTabEntitys.get(i).getCoverIcon());
            } else {
                publishIv.setVisibility(GONE);
            }
    ...
    private void updateTabSelection(int position) {
        for (int i = 0; i < mTabCount; ++i) {
            ...
            ImageView iv_tab_icon = tabView.findViewById(R.id.iv_tab_icon);
            ImageView iv_publish = tabView.findViewById(R.id.publish);
            CustomTabEntity tabEntity = mTabEntitys.get(i);

            if (mPublishMode == 1 && isSelect && mTabEntitys.get(i).getCoverIcon() != 0) {
                iv_publish.setVisibility(VISIBLE);
                iv_publish.setImageResource(mTabEntitys.get(i).getCoverIcon());
            }else {
    ...
    public void setmCenterPublishIcon(int mCenterPublishIcon) {
        this.mCenterPublishIcon = mCenterPublishIcon;
        updateTabStyles();
    }
    ...
    private void calcIndicatorRect() {
        ...
        TextView tv=currentTabView.findViewById(R.id.tv_tab_title);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (mUnderlineHeight > 0) {
            ...
            if (mUnderlineGravity == Gravity.BOTTOM) {
                ...
                if (mPublishMode==1&&mUnderlineGravity == Gravity.TOP) {
                    RelativeLayout rl=mTabsContainer.getChildAt(mCurrentTab).findViewById(R.id.rl_tap);
                    canvas.drawRect(paddingLeft, height -rl.getHeight()-mUnderlineHeight-getPaddingTop(), mTabsContainer.getWidth() + paddingLeft, height-rl.getHeight()-getPaddingTop(), mRectPaint);
                }else {
...
//            }
    ...
    //TypeEvaluator能够支持我们创建支持任意属性类型的动画。
    ...
    public void setFollowTvWidth(boolean followTvWidth){
        this.followTvWidth=followTvWidth;
    }
    ...
    public void showMsg(int position, int num,float leftPadding, float bottomPadding) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        ...
        if (tipView != null) {
            ...
            if (!mIconVisible) {
                setMsgMargin(position, leftPadding, bottomPadding);
            } else {
                setMsgMargin(position, 0,
                        mIconGravity == Gravity.LEFT || mIconGravity == Gravity.RIGHT ? 4 : 0);
            }
    ...
    public void showDot(int position, float leftPadding, float bottomPadding) {
        if (position >= mTabCount) {
            position = mTabCount - 1;
        }
        showMsg(position, 0, leftPadding, bottomPadding);
    }
